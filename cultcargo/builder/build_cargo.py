    ...
    if do_list:
        ...
        try:
            for image in sorted(imagenames):
                ...
                if mfile:
                    text = ""
                    # highlight actual version in bold (-ccX.Y.Z in normal text)
                    for v in found:
                        vv = v.rsplit("-", 1)
                        if len(vv) > 1:
                            text += f"**{vv[0]}**-{vv[1]}&nbsp;&nbsp;"
            ...
            if mfile:
                mfile.write("<br><br>*This file was automatically generated by build-cargo. Do not edit.*\n")
                mfile.close()
                if any_not_found:
